(ns slipway.security.oidc.client-credentials-flow
  (:require [clojure.tools.logging :as log]
            ...
            [slipway.security.oidc.jwt.at :as oidc.jwt.at]
...
(defn roles
  [access-token {::oidc.jwt/keys [user-roles-path]
...
(defn user-state
  [access-token {::oidc.jwt/keys [user-id-path]
                 :or             {user-id-path ["sub"]}
                 :as             opts}]
  (let [identity-fn     (::oidc/identity-fn opts)
    ...
    (cond-> {::principal/type    ::oidc/principal
...
(defmethod oidc/flow-handler :client-credentials
  [{::oidc/keys [issuer] :as opts}]
  (log/debug "initializing client credentials flow")
  (let [jwt-processor-bean (oidc.jwt.at/processor-bean opts)]
