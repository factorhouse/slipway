(ns slipway.security.oidc.user.principal
  (:require [clojure.tools.logging :as log]
            [slipway.principal :as principal]
            [slipway.security.oidc :as-alias oidc])
  (:gen-class
   :name slipway.security.oidc.user.principal.OpenIdUserPrincipalWithState
...
(defn redeem-refresh-token
  [{::oidc/keys [id-token response]} {:keys [refresh-token-fn user-state-fn]}]
  (if refresh-token-fn
    (if-let [refresh-result (refresh-token-fn id-token response)]
      (user-state-fn (or (::oidc/id-token refresh-result) id-token) ;; id-token refresh is optional
                     (::oidc/access-token refresh-result)
                     (::oidc/response refresh-result))
