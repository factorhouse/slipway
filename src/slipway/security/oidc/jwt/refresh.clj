(ns slipway.security.oidc.jwt.refresh
  ...
  (fn [original-id-token {:strs [refresh_token scope]}]
    ...
    (if-not (str/blank? refresh_token)
      (let [response     (-> (claim-refresh-token
                              ...
                              refresh_token)
                             check-response)
            id-token     (some-> (get response "id_token") oidc.jwt/decode) ;; id-token is optional in refresh response
            access-token (oidc.jwt/decode (get response "access_token"))] ;; access-token is required in refresh response
