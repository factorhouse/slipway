(ns slipway.security.oidc.authorization-code-flow
...
(defn user-state
  [id-token access-token response {::oidc/keys [identity-fn]
...
(defmethod oidc/flow-handler :default
  ...
  (let [oidc-config (oidc/configuration opts)]
    (doto (SecurityHandler$PathMapped.)
      (.setAuthenticator (authenticator oidc-config opts))
      (.setLoginService (OpenIdLoginService. oidc-config (login-service oidc-config opts)))
      (.setRealmName issuer))))
